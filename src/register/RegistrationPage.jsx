...
const RegistrationPage = (props) => {
  ...
  const backedUpFormData = useSelector(
    (state) => state.register.registrationFormData
  );
  const registrationError = useSelector(
    (state) => state.register.registrationError
  );
  const registrationErrorCode = registrationError?.errorCode;
  const registrationResult = useSelector(
    (state) => state.register.registrationResult
  );
  const shouldBackupState = useSelector(
    (state) => state.register.shouldBackupState
  );
  const userPipelineDataLoaded = useSelector(
    (state) => state.register.userPipelineDataLoaded
  );
  ...
  const optionalFields = useSelector(
    (state) => state.commonComponents.optionalFields
  );
  const thirdPartyAuthApiStatus = useSelector(
    (state) => state.commonComponents.thirdPartyAuthApiStatus
  );
  ...
  const providers = useSelector(
    (state) => state.commonComponents.thirdPartyAuthContext.providers
  );
  ...
  useEffect(() => {
    if (!userPipelineDataLoaded && thirdPartyAuthApiStatus === COMPLETE_STATE) {
      ...
      if (
        pipelineUserDetails &&
        Object.keys(pipelineUserDetails).length !== 0
      ) {
      ...
      return (
        <InstitutionLogistration
          ...
          headingTitle={formatMessage(
            messages["register.institution.login.page.title"]
          )}
        />
      );
    }
              ...
              <Form id="registration-form" name="registration-form">
                <NameField
                  ...
                  floatingLabel={formatMessage(
                    messages["registration.fullname.label"]
                  )}
                />
                <EmailField
                  name="email"
                ...
                {!currentProvider && (
                  <PasswordField
                    ...
                    floatingLabel={formatMessage(
                      messages["registration.password.label"]
                    )}
                  />
                )}
                <ConfigurableRegistrationForm
                  ...
                  setFieldErrors={
                    registrationEmbedded ? setTemporaryErrors : setErrors
                  }
                  setFormFields={setConfigurableFormFields}
                  autoSubmitRegisterForm={autoSubmitRegForm}
                  fieldDescriptions={fieldDescriptions}
                />
                {/* Mubeen code custom class add */}
